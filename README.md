...
- **对话模式** - 🆕 纯 LLM/VLM 对话模式，支持文本/图片多模态输入、流式输出、思考模式，无需 ADB 设备
...
对话模式是纯 LLM/VLM 对话模式，不涉及 GUI Agent 能力，适合纯文本/图片对话场景。

- **工作方式**：直接调用 LLM/VLM 进行对话，不操作设备
- **输入支持**：文本 + 图片多模态输入
- **输出特性**：支持流式输出、思考模式（`<think>...</think>` 标签）
- **适用场景**：纯对话、图片理解、文本生成等不需要设备操作的场景
- **特点**：无需 ADB 设备，配置简单，响应快速

**配置要点**：
- 在设置页面配置"对话模型"（独立于 GUI Agent 模型）
- 支持任何 OpenAI 兼容的 LLM/VLM API
- 对话历史自动保存在"任务历史"中
