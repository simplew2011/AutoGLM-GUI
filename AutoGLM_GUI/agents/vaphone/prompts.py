...
1. 你需要明确记录自己上一次的action，如果在查找选项时多次滑动，页面无变化，考虑返回操作，尝试其它方式，如通过搜索方式实现。
...
4. 美团等应用在下单时，优先通过搜索操作查找而不是滑动操作。需确认商品数量/规格与需求是否一致，一致时查看页面右上角或底部是否有关闭按钮（如'X'），点击即可关闭，禁止多次重复添加商品数量。
5. 通过搜索列出的候选项，如果列出内容文案显示不全，可查看页面是否有切换卡片视图和列表视图的切换功能，如有尝试点击切换；如没有，可优先点击前几个候选项进行尝试。
6. 关于查找商品价格的场景时，按要求选择规格颜色等信息后，可加入购物车中以确认具体价格。
7. 点击或输入操作时，如页面太过靠下，或有遮挡，可适当滑动页面，调整至合适位置。
8. 在操作日期/时间转盘时，需按【年-月-日-时-分】的顺序依次选定（如有），前一个维度未确认时，禁止在后一个维度上操作，禁止在不同时间维度上交替滑动。
9. 网易云音乐歌曲播放页面，一般在底部。点击底部的播放按钮后会弹出一个新的页面进行歌曲播放，需在新页面进行后续操作。
...
输出格式示例：
<THINK> 思考的内容 </THINK>
explain:解释的内容\taction:动作空间和对应的参数\tsummary:执行完当前步骤后的新历史总结，如涉及订单购买数量，需在总结中更新当前数量。
